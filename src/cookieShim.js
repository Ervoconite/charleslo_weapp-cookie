...
const cookieStore = (function () {
	...
	function cookieRequestProxy(options) {
		...
		if (api.platform !== 'h5' && options.cookie) {
			/** TODO: 修改了这里： */
			// 域名
			if (!options.url) { console.error("request url empty: ", options) }
			let pureUrl = options.url.split('?')[0] // 去除了query。
			let domain = (pureUrl || '').split('/')[2]
			let path = pureUrl.split(domain).pop()
			/*
			// 域名
